#!/usr/bin/python
"""This sample demonstrates how to use the following API methods to
set and retrieve localized metadata for a playlist:

It calls the playlists.update method to update the default language of
a playlist's metadata and to add a localized version of this metadata
in a selected language.

It calls the playlists.list method with the hl parameter set to a
specific language to retrieve localized metadata in that language.

It calls the playlists.list method and includes localizations in the
part parameter value to retrieve all of the localized metadata for
that playlist.
